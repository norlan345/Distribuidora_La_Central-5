...
namespace Distribuidora_La_Central.Web.Controllers
{
    ...
    public class ProductoController : ControllerBase
    {
        ...
        public IActionResult BuscarPorDescripcion([FromQuery] string descripcion)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(descripcion))
                {
                    return BadRequest(new { StatusCode = 400, Message = "El parámetro 'descripcion' es requerido" });
                }
                ...
                return NotFound(new { StatusCode = 404, Message = "No se encontraron productos con la descripción proporcionada" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { StatusCode = 500, Message = "Error al buscar productos por descripción", Error = ex.Message });
            }
        }
